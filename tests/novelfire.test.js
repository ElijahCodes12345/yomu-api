...
const BASE_URL = 'http://localhost:3000/api/novelfire';

async function testNovelFire() {
  console.log('🧪 Testing NovelFire API endpoints...\n');
  ...
  try {
    ...
    if (novelInfo && novelInfo.title) {
      console.log(`✅ Info PASS: Title: "${novelInfo.title}", Author: "${novelInfo.author}", Genres: [${novelInfo.genres?.join(', ')}]`);
    } else {
    ...
    console.log('\n🎉 ALL NOVELFIRE TESTS COMPLETED SUCCESSFULLY!');
  } catch (error) {
